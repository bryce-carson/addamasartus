...
#+begin_src R
  cyber.bak <- fs::path_home("src", "XBOX", "ogxboxBackups", "Cyber BACKUP.ess")
#+end_src

#+RESULTS:
: /var/home/bryce/src/XBOX/ogxboxBackups/Cyber BACKUP.ess
...
I am working on this project in my free time to practice "application
programming." The literate paradigm is followed to provide implementation
documentation so that the API is as useful as possible to others.

** The ESX reader
Reading ESX files is not too complex. The format is documented well-enough in
[[file:mw_esm.txt][Dave Humphrey's "mw_esm.txt"]]. An ESS, ESP, or ESM file is read by creating a new
/ESX object/ with the ~ESX~ function: in turn, some simple object field
initialization occurs then the ~read~ method is called. The method reads the
binary Elder Scrolls file described by the =con= method argument.

#+begin_src R
  ## 1. Create an ESX object.
  ESX(fs::path_home("Saved Games", "Morrowind", "Narevarine", "agronian_guy.ess"))
#+end_src

ESX files are objects composed of Records, themselves composed of Subrecords.

#+begin_src R :noweb yes :tangle R/read.R
...
The composition is read "lazily", only reading the TES3 header Record at first,
and subsequent calls to the read method on an ESX object will either "enumerate"
the rest of the records (parsing them immediately), or read the records "lazily"
(only reading the headers of the records).

See the first ~setMethod~ call in the following chunk for /what the ~read~
method of the =ESX= class/ does. The method is called with the following arguments:

- con :: an fs_path created from the input string.
- parsing_method :: =TES3=, so only the TES3 header of the Elder Scrolls file is
  enumerated; enumeration of records ceases after reading the TES3 record to
  minimize up-front effort.
- filter :: =TES3=, only records matching this name are parsed, other records
  are only enumerated.

#+begin_src R :noweb-ref "ESX.read File"
  #' @param how dictactes *how* the ESX file is parsed. This argument
  ...
  #'   types are unparsed (reading only their header).
  setMethod("read", "ESX", function(x, con, how = c("TES3", "ENUM", "LAZY"), filter = "TES3") {
    ...
    if (inherits(maybeTES3, "try-error") || length(maybeTES3) < 4) {
      stop("Error reading from file connection, or less than four bytes were read.")
    }
   ...
   functions do nothing when reading lazily, so the filter is ignored.")
    ...
    ## names(records[[1]]) <- "TES3"
    ## Error in names(records[[1]]) <- "TES3" (from read.R#114) :
    ##   invalid to use names()<- on an S4 object of class 'Record'
    x@records <- records
...
#+begin_src R :noweb-ref "ESX.read Record"
...
** Dispatching to ESX Record-parsing methods using metaprogramming
Metaprogramming is used to look at what parsing methods exist; if a method
doesn't exist for a given record type an error is returned. This allows for an
iterative development process, and as more parsing methods are written during a
development session existing objects benefit from the use of metaprogramming
(called reflection in Java and some other languages).

#+begin_src R :noweb-ref "ESX.read Subrecord"
...
** ESX Record Parsing Methods
Record parsing functions are defined herein.

Float values are stored specially.

#+begin_src R :tangle R/FLTV.R
...
HEDR subrecord, a special subrecord used only in the TES3 record type.

#+begin_src R :tangle R/HEDR.R
...
MAST subrecord, a special subrecord used only in the TES3 record type.

#+begin_src R :tangle R/MAST.R
...
#+begin_src R :tangle R/S4ESX.R
  ...
  print.ESX <- function(x, ...) {
    requireNamespace("cli")

    cli::cli_h1("ESX @ {.path {x@path}}")
    for (r in x@records) {
      cli::cli_h2("Record '{r@name}")
      cli::cli_bullets(c("*" = "Offset = {r@offset}",
                         "*" = "Size (bytes) = {r@size}"))
      print(r)
    }
  }

  setMethod("print", "ESX", function(x, ...) {
    print.ESX(x, ...)
  })

  print.Record <- function(x, ...) {
    for (sr in x@subrecords) {
      print(sr)
    }
  }

  setMethod("print", "Record", function(x, ...) {
    print.Record(x, ...)
  })
...
#+begin_src R :tangle R/S4Record.R
...
#+begin_src R :tangle R/S4Subrecord.R
...
Screenshot record data.

#+begin_src R :tangle R/SCRS.R
...
A parser error messaging helper function.

#+begin_src R :tangle R/parseError.R
...
* Demonstration Shiny application
The Addamasartus package provides an API for interacting with ESS, ESP, and ESM
files for TES III: Morrowind. This demonstration Shiny application utilizes the
API to create an application for viewing the S4 *ESX* object created from
parsing an Elder Scrolls file. The view of the object is either a tree or a
graph. The code was initially AI-generated, using Shiny Assistant. Once
frustration with the AI reached a maximum the code was exported and hand-written
therefrom.

This demonstration application is incomplete and will probably be deleted in
favour of a simpler GUI approach utilizing Tcl/Tk.

#+begin_src R :tangle R/shiny.R
