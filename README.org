...
/ESx object/ with the ~ESX~ function; immediately the ~read~ method of an ESx
object is called to read the file. See the first ~setMethod~ call in the
following chunk.

#+begin_src R :noweb :tangle R/read.R
